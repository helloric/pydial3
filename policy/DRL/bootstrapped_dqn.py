...
class DeepQNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, \
        ...
        for head in range(no_head):
            ...
            self.pred_q.append(tf.reshape(tf.reduce_sum(input_tensor=self.heads[head] * actions_one_hot, axis=1, name='q_acted'),
        ...
        self.loss = []
        for head in range(no_head):
            self.loss.append(tf.reduce_mean(input_tensor=self.clipped_error(self.diff[head]), name='loss'))
